...
LOk.Left := (ClientWidth div 2)-(LOk.Left div 4);
...
begin
 ...
 begin
  ...
  begin
    ...
    if (I = 2) or (I = 3) or (I = 6) or (I = 7) or (I = 10) or (I = 11) or (I = 14) or (I = 15)  then
    inc(ALL);
  ...
  begin
    LAnsw.Caption := RGYesNo.Items[1];
        if (I = 1) or (I = 4) or (I = 5) or (I = 8) or (I = 9) or (I = 12) or (I = 13) or (I = 16)  then
    inc(ALL);
    end;
 ...
 Randomize;
  ...
  begin
   ...
   ADOQParamRes.SQL.Add('SELECT * FROM ParamResult WHERE Parametr = '+QuotedStr('Првильных ответов'));
   // активировать запрос
   ADOQParamRes.Active := True;
  ...
  begin
   ...
   ADOQParamRes.SQL.Clear;
   // найти номер соответствующего параметра результатов
   ADOQParamRes.SQL.Add('SELECT * FROM ParamResult WHERE Parametr = '+QuotedStr('Время'));
   // активировать запрос
   ADOQParamRes.Active := True;
   // считать код параметра
   Code_Param := ADOQParamRes.FieldByName('ID_ParamResult').AsInteger;
   ...
   ADOQRes.SQL.Clear;
   // записать в результаты тестирования номера эксперимента, параметра результата, значение результата
   ADOQRes.SQL.Add('INSERT INTO Result (ID_Testing, ID_ParamResult, Value_Result)VALUES('+IntToStr(ID_Testing)+' ,'+IntToStr(Code_Param)+', '+IntToStr(t)+')');
   // выполнить запрос
   ADOQRes.ExecSQL;
  end;
...
begin
 ...
 // подготовить очередной вопрос}
  ...
  begin
   ...
   Image1.Picture.LoadFromFile(S+'\Materials\'+LQuest.Caption);
  end;
  end;
